...
class HICODetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'hoi_id': torch.tensor(s['hoi_id'], dtype=torch.long),
...
class ObjectDetector(nn.Module):
    def __init__(self, score_thresh=0.3):
        super().__init__()
...
class TwoPassHOIDetector(nn.Module):
    def __init__(self, num_classes=1, pos_weight = 5.0):
        super().__init__()
...
def evaluate(model, loader, device):
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        # Log label distribution for first batch of each epoch
        if batch_idx == 0:
            print(f"  First batch of epoch: "
                  f"{int(labels.sum())} throws / {len(labels)} samples")

        total_throws += labels.sum().item()
...
def build_loaders(hico_root, vcoco_root = None, coco_root = None, batch_size=64, neg_ratio = 1):
    ...
    # --- Weighted sampler so each batch is ~50/50 throw vs non-throw ---
    train_labels = [s['label'] for s in hico_train.samples]
    n_throw     = sum(train_labels)
    n_nonthrow  = len(train_labels) - n_throw

    print(f"Throw samples    : {n_throw}")
    print(f"Non-throw samples: {n_nonthrow}")
    assert n_throw > 0, "No throwing samples found — check THROW_HOI_IDS"

    # Each throwing sample gets weight inversely proportional to its class count
    weights = [
        (1.0 / n_throw)    if l == 1.0
        else (1.0 / n_nonthrow)
        for l in train_labels
    ]
    weights = torch.tensor(weights, dtype=torch.float)

    # neg_ratio negatives per throw — gives more variety than pure 50/50
    num_samples = int(n_throw * (1 + neg_ratio))
    print(f"Samples per epoch: {num_samples}  "
          f"({n_throw} throws + ~{n_throw * neg_ratio} non-throws)")
    print(f"Batches per epoch: {num_samples // batch_size}")
    ...
    print(f"Train samples total : {len(hico_train.samples)}")
    print(f"  throw             : {int(n_throw)}")
    print(f"  non-throw         : {int(n_nonthrow)}")
    print(f"Samples per epoch   : {num_samples} (balanced)")
...
def train(train_loader, val_loader, epochs=5, lr=1e-4, checkpoint_dir='/content/drive/MyDrive/checkpoints'):
...
if __name__ == '__main__':
    train_loader, val_loader = build_loaders('/content/drive/MyDrive/hico_20160224_det')
